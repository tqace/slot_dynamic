...
class SlotAttentionModel(nn.Module):
    ...
    def forward(self, x, force_slots=None):
        batch_size ,num_channels, height, width = x.shape

        if force_slots is not None:
            decoder_in = force_slots.repeat(1, 1, self.decoder_resolution[0], self.decoder_resolution[1])

            out = self.decoder_pos_embedding(decoder_in)
            out = self.decoder(out)
            # `out` has shape: [batch_size*num_slots, num_channels+1, height, width].
            #assert_shape(out.size(), (batch_size*num_slots, num_channels + 1, height, width))
            b_n, slot_size,_,_ = force_slots.shape
            num_slots = int(b_n/batch_size)
            force_slots=force_slots.view(batch_size,num_slots,slot_size)
            ...
            return recon_combined, recons, masks, force_slots
